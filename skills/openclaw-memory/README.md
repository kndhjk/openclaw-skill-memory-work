...
On Termux, native module builds can fail because `node-gyp` expects an Android NDK path variable during `better-sqlite3` setup.
Use the included installer instead of plain `npm install` inside the skill folder:
...
This installer creates a local compatibility path and exports `npm_config_android_ndk_path` only for the install command, which is more stable than relying on `.npmrc` hacks.
